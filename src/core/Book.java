...
public class Book {
   ...
   for(int i=0; i<articles.length;i++) {
    ...
    public void setTitle(String title) {
    	...
    	String [] array = title.split(" ");//"the Chronicles OF TrumP"
    	
    	
    	for (int i = 0; i < array.length; i++) {
    		if ( i == 0)
    			newStr += capitalize(array[i])+ " "; //The Chronicles of  		
    		else if ( isTitleWord(array[i]) )
    			newStr += capitalize(array[i])+ " ";
    		else 
   ...
   //to String() method is a special that is normally describe the object;
